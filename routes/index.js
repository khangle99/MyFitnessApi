  ...
  function (req, res, next) {
    ...
    if (!req.file) {
      ...
      categorysRef
        ...
        .catch((error) => {
          console.error("Loi tao moi excercise category: ", error);
          fs.unlinkSync("./" + req.file.path);
          res.status(500).json({ error: "loi set excercise category" });
          res.end();
        });
    }
  }
  ...
  function (req, res, next) {
    ...
    if (!req.files) {
      ...
      req.files.forEach((img) =>
        imgUrls.push("https://" + req.hostname + img.path.substring(6))
      );
      // cap nhat firestore
      ...
      categoryRef
        ...
        .then((doc) => {
          if (!doc.exists) {
            ...
            let oldPhotoPath =
              "./public/data/uploads/" +
              photoUrl.substring(photoUrl.indexOf("uploads/") + 8);
            try {
              if (imgUrls.length != 0) {
                categoryRef
                  .update({
...
router.delete("/deleteExcerciseCategory", function (req, res, next) {
  let id = req.query.id;
  // kiem tra ton tai
        ...
        .then((doc) => {
          if (!doc.exists) {
            ...
            photoUrls.forEach((url) =>
              oldPhotoPaths.push(
                "./public/data/uploads/" +
                url.substring(url.indexOf("uploads/") + 8)
              )
            );
                ...
                excerciseRef
                  ...
                  .then(() => {
                    try {
                      ...
                      console.log("khong xoa file thanh cong");
                      res
                        .status(500)
                        .json({ error: "loi khong doc duoc firestore" });
                      res.end();
                    }
                  });
              } else {
    ...
    if (!req.file) {
      ...
      admin
        ...
        .catch((error) => {
          ...
          fs.unlinkSync("./" + req.file.path);
          res.status(500).json({ error: "loi set nutrition category" });
          res.end();
        });
    }
  }
              ...
              categoryRef
                ...
                .then(() => {
                  try {
                    ...
                    console.log("loi xoa nutrition photo")
    ...
    .then((doc) => {
      if (!doc.exists) {
        ...
        categoryRef
          ...
          .catch((error) => {
            res.status(500).json({ error: "loi xoa nutrition firestore" });
            res.end();
          });
      }
    })
